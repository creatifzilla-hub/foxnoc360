...
for user in users_to_test:
    ...
    print(f"\n=== Testing for {user['email']} (role: {user['role']}, tenant: {user['tenant_id']}) ===")
    ...
    try:
        with urllib.request.urlopen(req) as response:
            ...
            print("Dashboard:", json.dumps(res_data, indent=2))
    ...
    try:
        with urllib.request.urlopen(req_table) as response:
            ...
            for d in res_data:
                print(f"  - Name: {d['device_name']} | IP: {d['ip_address']} | Status: {d['latest_status']} | Tenant: {d.get('tenant_name')}")
    except Exception as e:
